        ...
        const std::string      &environmentVariable)
{
    ...
    // prepare the .tpr file
    {
        ...
        auto mdpString = prepareMdpFileContents(mdpFieldValues);
        runner->useStringAsMdpFile(mdpString);
        EXPECT_EQ(0, runner->callGrompp(caller));
    }

    // save state of environment variable
    auto environmentVariableBackup = getenv(environmentVariable.c_str());
    ...
    // do the second mdrun
    {
        ...
        setenv(environmentVariable.c_str(), "ON", true);
        ASSERT_EQ(0, runner->callMdrun(simulator2Caller));
    }

    if (environmentVariableBackup != nullptr)
    {
        setenv(environmentVariable.c_str(), environmentVariableBackup, true);
    }
    else
    {
        unsetenv(environmentVariable.c_str());
